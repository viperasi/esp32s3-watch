...
lv_obj_t *app_launcher_create(void)
{
    ...
    int grid_w = SCR_W - PAD * 2;
    int grid_h = SCR_H - TOP - BOT;
    int cell_w = (grid_w - (COLS - 1) * PAD) / COLS;
    int cell_h = (grid_h - (ROWS - 1) * PAD) / ROWS;

    /* Grid container: holds rows of cells */
    lv_obj_t *grid = lv_obj_create(scr);
    lv_obj_set_size(grid, SCR_W, grid_h);
    lv_obj_align(grid, LV_ALIGN_TOP_MID, 0, TOP);
    ...
    for (int i = 0; i < cells; i++) {
        ...
        /* Content: column layout with icon on top, name below */
